...
export default function KDash() {
  ...
  // Fetch the total voters count
  const fetchVotersCount = async () => {
    try {
      const response = await axios.get(`http://192.168.1.8:8000/api/voters_by_group_user/${KuserId}/`);
  ...
  const fetchNonVotedCount = async () => {
    try {
      const response = await axios.get(`http://192.168.1.8:8000/api/voter_status/${KuserId}/2/`);
