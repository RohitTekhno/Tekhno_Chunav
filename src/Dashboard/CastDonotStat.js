...
const CastDonutStat = () => {
    ...
    const getReligionwiseData = async () => {
        try {
            ...
            const data = [
                { label: 'Hindu', value: result.data.Hindu || 0 },
                { label: 'Muslim', value: result.data.Muslim || 0 },
                { label: 'Not Defined', value: result.data['Not Defined'] || 0 }
            ];
    ...
    const handlePress = (index) => {
        const selectedSlice = sliceData[index];
        Alert.alert(
            `Statistics`,
    ...
    const renderOverlay = () => {
        const sliceWidth = widthAndHeight / 2;
        return sliceData.map((_, index) => (
            <TouchableWithoutFeedback key={index} onPress={() => handlePress(index)}>
                <View style={{
                    position: 'absolute',
                    width: sliceWidth,
                    height: sliceWidth,
                    borderRadius: sliceWidth,
                    top: 0,
                    left: 0,
                    zIndex: 1,
                }} />
        ...
        <View style={styles.container}>
            ...
            <View style={styles.legendContainer}>
                <View style={styles.legendColumn}>
                    <View style={styles.legendItem}>
                        <View style={[styles.legendColor, { backgroundColor: '#F8700F' }]} />
                        <Text style={styles.legendLabel}>Hindu</Text>
                    </View>
                    <View style={styles.legendItem}>
                    ...
                    <View style={styles.legendItem}>
                        ...
                        <Text style={styles.legendLabel}>Not Defined</Text>
                    </View>
                </View>
            </View>
        </View>
