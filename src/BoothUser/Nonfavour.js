...
export default function Against({ route, navigation }) {
  const { redRecords } = route.params;
  ...
  const renderVoterItem = ({ item }) => (
    <View style={[styles.voterItem, { backgroundColor: getBackgroundColor(item.color || 'red') }]}>
  ...
  const getBackgroundColor = (color) => {
    switch (color) {
      ...
      case 'yellow':
        return '#fff3cd';
      default:
        return '#f8d7da'; // Default background color for red records
    }
  };
